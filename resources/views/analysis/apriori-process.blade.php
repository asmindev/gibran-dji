        ...
        <div class="p-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full table-auto">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($step['data'] as $item)
                        <tr class="{{ $item['status'] === 'pruned' ? 'bg-red-50' : 'bg-green-50' }}">
                            <td class="px-4 py-2 text-sm font-medium text-gray-900">{{ $item['item'] }}</td>
                            <td class="px-4 py-2 text-sm">{{ $item['support'] }}%</td>
                            <td class="px-4 py-2 text-sm">
                                @if($item['status'] === 'kept')
                                <span
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    Z <div class="flex flex-wrap gap-1 items-center">
                        @php
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full table-auto">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($step['data'] as $itemset)
                        {{-- check the status --}}
                        @if($itemset['status'] === 'kept')
                        <tr class="{{ $itemset['status'] === 'pruned' ? 'bg-red-50' : 'bg-green-50' }}">
                            ...
                            <td class="px-4 py-2 text-sm">{{ $itemset['count'] }}/{{
                                $algorithmSteps['summary']['total_transactions'] }}</td>
                            <td class="px-4 py-2 text-sm">{{ $itemset['support'] }}%</td>
                            <td class="px-4 py-2 text-sm">
                                @if($itemset['status'] === 'kept')
                                <span
                                    class="inline-flex px-2 py-1 text-xs font-semibold bg-green-100 text-green-800 rounded-full">
...
<div class="bg-white rounded-lg shadow-md p-6 mt-8">
    ...
    <div class="grid md:grid-cols-5 gap-6">
        ...
        <div class="text-center">
            <div class="text-3xl font-bold text-green-600">{{ $algorithmSteps['summary']['frequent_1_itemsets'] }}
            </div>
            <div class="text-sm text-gray-600">Frequent 1-Itemsets</div>
        </div>
        <div class="text-center">
            <div class="text-3xl font-bold text-purple-600">{{ $algorithmSteps['summary']['frequent_2_itemsets'] }}
            </div>
