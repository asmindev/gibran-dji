...
<div class="bg-white shadow rounded-lg p-6">
    ...
    <div class="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <ul class="text-sm text-blue-800 space-y-1">
            ...
            <li>• Kode barang harus sudah ada dalam sistem</li>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3 flex-1">
                        ...
                        <div class="mt-2 text-sm text-red-700">
                            ...
                            <div class="mb-3">
                                <h4 class="font-medium text-red-800">Kode Barang Tidak Ditemukan:</h4>
                        ...
                        <div class="mt-3">
                            <div class="bg-blue-50 border border-blue-200 rounded p-2">
                                <p class="text-xs text-blue-700">
                                    <strong>Tips:</strong>
                                    @if(isset(session('import_errors')['not_found']))
                                    Pastikan kode barang sudah terdaftar di master data.
                                    @endif
                                    @if(isset(session('import_errors')['stock']))
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tanggal Keluar</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <tr>
                    ...
                    <td class="px-4 py-2 text-sm">BRG001</td>
                    <td class="px-4 py-2 text-sm">03/08/2025</td>
                    <td class="px-4 py-2 text-sm">2</td>
                    <td class="px-4 py-2 text-sm">Kondisi baik</td>
                </tr>
                <tr>
                    <td class="px-4 py-2 text-sm">2</td>
                    <td class="px-4 py-2 text-sm">TRX20250803002</td>
                    <td class="px-4 py-2 text-sm">BRG002</td>
                    <td class="px-4 py-2 text-sm">03/08/2025</td>
                    <td class="px-4 py-2 text-sm">1</td>
                    <td class="px-4 py-2 text-sm">ID Transaksi wajib diisi</td>
                </tr>
            </tbody>
        </table>
    </div>
...
function displayPreview(data) {
    ...
    if (previewData.length > 0) {
        ...
        const expectedHeaders = ["No","ID Transaksi", "Kode Barang", "Nama Barang", "Kategori", "Tanggal Keluar", "Jumlah", "Catatan"];
    ...
    for (let i = 1; i < previewData.length && i <= 6; i++) {
        ...
        row.forEach((cell, cellIndex) => {
            ...
            const kodeBarangIndex = headerMapping["Kode Barang"];
            const tanggalIndex = headerMapping["Tanggal Keluar"];
            ...
            if (cellIndex === kodeBarangIndex && !trimmedValue) cellClass += ' bg-red-100'; // Kode barang required
...
function validateRow(row, rowIndex, headerMapping) {
    ...
    const kodeBarangIndex = headerMapping["Kode Barang"];
    if (kodeBarangIndex !== undefined) {
        const kodeBarang = row[kodeBarangIndex] ? String(row[kodeBarangIndex]).trim() : '';
        if (!kodeBarang) {
            hasErrors = true;
        }
    } else {
    ...
    if (tanggalIndex !== undefined) {
        const tanggal = row[tanggalIndex] ? String(row[tanggalIndex]).trim() : '';
        if (!tanggal) {
            hasErrors = true;
        }
    } else {
...
function validateAllData(data, headerMapping = {}) {
    ...
    const requiredHeaders = ["ID Transaksi", "Kode Barang", "Tanggal Keluar", "Jumlah"];
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        // Kode barang validation
        const kodeBarangIndex = headerMapping["Kode Barang"];
        if (kodeBarangIndex !== undefined) {
            const kodeBarang = row[kodeBarangIndex] ? String(row[kodeBarangIndex]).trim() : '';
            if (!kodeBarang) {
                errors.push(`Baris ${i}: Kode barang tidak boleh kosong`);
            }
        }
        ...
        if (tanggalIndex !== undefined) {
            const tanggal = row[tanggalIndex] ? String(row[tanggalIndex]).trim() : '';
            if (!tanggal) {
                errors.push(`Baris ${i}: Tanggal keluar tidak boleh kosong`);
            }
        }
...
function showImportError(errors, errorCount) {
    ...
    if (errors.not_found && errors.not_found.length > 0) {
        ...
        errorHTML += '<h4 class="font-medium text-red-800">Kode Barang Tidak Ditemukan:</h4>';
    ...
    if (errors.not_found && errors.not_found.length > 0) {
        errorHTML += 'Pastikan kode barang sudah terdaftar di master data. ';
    }
    if (errors.stock && errors.stock.length > 0) {
